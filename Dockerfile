...
RUN pip install --no-cache-dir "faster-whisper==1.*" "pyannote.audio==3.3.2" "huggingface_hub<1.0" "matplotlib" \
      "nvidia-cublas-cu12" "nvidia-cudnn-cu12==9.*" pillow pytesseract
WORKDIR /app
COPY worker.py vision.py entrypoint.sh /app/
...
# Import smoke test: a missing module (matplotlib, a hub kwarg) must fail the build, not a pod.
RUN python -c "from pyannote.audio import Pipeline; from faster_whisper import WhisperModel; import torch; print('imports ok, torch', torch.__version__)"
